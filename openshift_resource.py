...
#  def patch_applied(self, namespace, kind, name, current, patch, path = ""):
  ...
  def run_deployer(self):
    ...
    if self.arguments:
      args = " -p " + ",".join("=".join(_) for _ in self.arguments.items())
    else:
      args = ""

    #if deploy_needed:
    #self.apply_template(self.deployer, self.arguments)
    (rc, stdout, stderr) = self.module.run_command('oc new-app ' + self.deployer + args, check_rc=True)
 #jsonpath='{.status.containerStatuses[?(@..exitCode)]..exitCode}'
    #return changed
